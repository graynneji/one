...
// import { useCrudCreate, useGetById, useRpc, useUpdateCrud } from '@/hooks/useCrud';
...
//     const updateAppointmentMutation = useUpdateCrud("appointment", [["messages"], ["appointment"]]);
...
//                     : event
//             ));
//             updateAppointmentMutation.mutateAsync(events, )
//         } else {
//             // Add new event
...
import { useCheckAuth } from '@/context/AuthContext';
import { useColorScheme } from '@/hooks/useColorScheme';
import { useCrudCreate, useDeleteCrud, useGetById, useRpc, useUpdateCrud } from '@/hooks/useCrud';
import { sendMessage } from '@/types';
import { capitalizeFirstLetter, formatDate, formatDateTime } from '@/utils';
import { generatePatientId } from '@/utils/uniqueId';
import { Ionicons } from '@expo/vector-icons';
...
const Schedule: React.FC = () => {
    ...
    const createAppointmentMutation = useCrudCreate("appointment", [["messages"], ["appointment"]]);
    const createMessageMutation = useCrudCreate<sendMessage>("messages");
    const updateAppointmentMutation = useUpdateCrud("appointment", [["messages"], ["appointment"]]);
    const deleteNoteMutation = useDeleteCrud("appointment", [["messages"], ["appointment"]]);
    const rpcViewMutation = useRpc("increment_patient_session", ["patients"]);
    ...
    const { data: therapistIdPatientUser, isLoading } = useGetById(
        "user",
        { user_id: userId },
        "therapist(name, therapist_id)",
        !!userId && !isTherapist,
    ...
    // Fetch therapist data
    const { data: therapistData } = useGetById(
        "therapist",
        { therapist_id: userId },
        "id",
        !!userId && isTherapist,
    ...
    // Fetch patients (only for therapists)
    const { data: patientsData } = useGetById(
        "patients",
        { therapist: therapistId },
        "id, name, patient_id, created_at",
        !!therapistId,
    ...
    const mergeDateTime = (date: string, time: Date): string => {
        const [year, month, day] = date.split("-").map(Number);
        const merged = new Date(year, month - 1, day);
    ...
    const saveEvent = async () => {
        try {
            ...
            if (editingEvent) {
                ...
                const result = await createAppointmentMutation.mutateAsync(newEvent);

                if (!isTherapist) {
                    await rpcViewMutation.mutateAsync({
                        p_patient_id: userId
                    });
                } else {
                    await rpcViewMutation.mutateAsync({
                ...
                const receiverId = !isTherapist
                    ? therapistIdPatientUser?.result[0]?.therapist?.therapist_id
                    : selectedPatient;

                if (receiverId && result?.data?.id) {
                    const insertSchToMessage = {
                        sender_id: userId,
                    ...
                    await createMessageMutation.mutateAsync(insertSchToMessage);
                }
            }
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            setEvents(prev => prev.filter(event => event.id !== eventId));
                            deleteNoteMutation.mutate(eventId as string | number);
                            await refetch();
                        } catch (err) {
                ...
                <SafeAreaView style={styles.modalContainer}>
                    <View style={styles.modalHeader}>
                        ...
                        <TouchableOpacity
                            ...
                            disabled={isSaving}
                        >
                            <Text style={[styles.saveButtonText, isSaving && { color: colors.placeholder }]}>
